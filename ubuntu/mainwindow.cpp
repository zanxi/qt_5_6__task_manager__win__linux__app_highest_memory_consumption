    ...
    ui(new Ui::MainWindow)
{
    ...
    HStrList.push_back(QString("PID")); // 0
    HStrList.push_back(QString("Name")); // 1
    HStrList.push_back(QString("PPID")); // 2
    HStrList.push_back(QString("PGID")); // 3
    HStrList.push_back(QString("Mem(KiB)")); // 4
    HStrList.push_back(QString("Priority")); // 5
...
void MainWindow::SortMemory()
{
    ...
    for(int k = 0; k < PID.size(); ++k){
        QStringList proc_data = ProcTool::getProcInfo(PID.at(k));
        if(proc_data.size()==0)continue;
        QVector<QString> proc_pid_data;
        for(int i = 0; i < 6; ++i)
        {
            proc_pid_data.push_back((proc_data[i]));
            //qDebug()<<" | "<<proc_data[4]<<" | ";
            //qDebug()<<proc_data[0]<<" | "<<proc_data[1]<<" | "<<proc_data[2]<<" | "<<proc_data[3]<<" | "<<proc_data[4]<<" | "<<proc_data[5];
        }
        map.insert(QString(proc_data[5]).toInt(),proc_pid_data);
    }

    QList<long long> keys = map.keys();

    int count = 0;
    for(int k = keys.size()-1; k>=0;--k) //keys.size()-7; --k)
    {
    //foreach (QVector<QString> v, map.) {
        QVector<QString> proc_data = map.take(k);
        if(proc_data.size()==0)continue;
        count++;
        for(int i = 0; i < 6; ++i){
            qDebug()<<" | "<<proc_data[4]<<" | ";
            //qDebug()<<"|||"<<map.keys()[5]<<"|||"<<proc_data[0]<<" | "<<proc_data[1]<<" | "<<proc_data[2]<<" | "<<proc_data[3]<<" | "<<proc_data[4]<<" | "<<proc_data[5];
        }
        if(count>7) break;
    }
...
void MainWindow::update2()
{
    ...
    QList<long long> keys = map.keys();
    for(int k = keys.size()-1; k>=keys.size()-7; --k)
    {
    //foreach (QVector<QString> v, map.) {
        QVector<QString> proc_data = map.take(k);
        if(proc_data.size()==0)continue;
        for(int i = 0; i < 6; ++i){
            ...
            if(i == 0){
                ...
                item->setData(2,QString(proc_data[i]).toInt());
                ui->tableWidget->setItem(k, i,item);
                ui->tableWidget->item(k,i)->setTextAlignment(Qt::AlignCenter);
            }
            else ui->tableWidget->setItem(k, i, new QTableWidgetItem(proc_data[i]));
            ui->tableWidget->item(k,i)->setTextAlignment(Qt::AlignCenter);
        }
    }
...
//    }
    ...
    for(int k = 0; k < PID.size(); ++k){
        ...
        for(int i = 0; i < 6; ++i){
            if(i == 0){
                QTableWidgetItem* item = new QTableWidgetItem();
                item->setData(Qt::DisplayRole,QString(process[i]).toInt());
                ui->tableWidget->setItem(k, i,item);
                ui->tableWidget->item(k,i)->setTextAlignment(Qt::AlignCenter);
            }
            if(i == 4){
    ...
    for(int i=0;i<6;i++)
    {
        QString proc_pid = ui->tableWidget->model()->index(i,0).data().toString();
        QString proc_name = ui->tableWidget->model()->index(i,1).data().toString();
        QString proc_memory = ui->tableWidget->model()->index(i,4).data().toString();
