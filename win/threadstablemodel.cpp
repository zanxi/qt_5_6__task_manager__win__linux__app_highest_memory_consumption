...
void ThreadsTableModel::addThreadToList(PTHREADENTRY32 p, THREAD_STATE s, FILETIME kernel_t, FILETIME user_t)
{
    ...
    QBarSet* kernelTimeBarSet = getKernelTimeBarSetForPid(pid);

    // Добавляем в QBarSet значение kernel time для очередного потока:
    *kernelTimeBarSet << ti.kernelTime;

    QBarSet* userTimeBarSet = getUserTimeBarSetForPid(pid);
...
{
    QMap<DWORD, QStackedBarSeries*>::const_iterator stackedBarSeriesIt = stackedBarSeriesMap.find(pid);
...
{
    QMapIterator<DWORD, QStackedBarSeries*> stackedBarSeriesIt(stackedBarSeriesMap);
    while (stackedBarSeriesIt.hasNext()) {
        stackedBarSeriesIt.next();
        QStackedBarSeries* stackedBarSeries = stackedBarSeriesIt.value();
...
// Получить время ЦП проведенное в режиме ядра:
QBarSet* ThreadsTableModel::getKernelTimeBarSetForPid(DWORD pid)
{
    QMap<DWORD, QBarSet*>::const_iterator it = kernelTimeMapPidBarSet.find(pid);

    QBarSet *barSet;
    if (it == kernelTimeMapPidBarSet.end())
    {
        barSet = new QBarSet("Время ядра ЦП (с)");
...
// Получить время ЦП проведенное в режиме пользователя:
QBarSet* ThreadsTableModel::getUserTimeBarSetForPid(DWORD pid)
{
    QMap<DWORD, QBarSet*>::const_iterator it = userTimeMapPidBarSet.find(pid);

    QBarSet *barSet;
    if (it == userTimeMapPidBarSet.end())
    {
        barSet = new QBarSet("Время пользователя ЦП (с)");
