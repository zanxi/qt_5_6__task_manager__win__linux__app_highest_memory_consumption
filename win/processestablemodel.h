...
public:
    ...
    QMapIterator<DWORD, QLineSeries*> getMemSeriesIterator();

    // Получить серии данных о потреблении памяти заданным процессом:
    QLineSeries* getMemSeriesForPid(DWORD pid);
    ...
    QLineSeries* getThreadCountSeriesForPid(DWORD pid);
...
private:
    ...
    QMap<DWORD, QLineSeries*> memSeries;

    // Серии данных о количестве потоков:
    QMap<DWORD, QLineSeries*> threadCountSeries;
