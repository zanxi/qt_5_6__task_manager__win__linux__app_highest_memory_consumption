...
void ProcessesTableModel::addTaskToList(PPROCESSENTRY32 p, SIZE_T mem, DWORD pri)
{
     ...
     QMap<DWORD, QLineSeries*>::const_iterator valueIt = memSeries.find(pid);
     QLineSeries *value;
     if (valueIt == memSeries.end())
     {
         value = new QLineSeries();
...
QMapIterator<DWORD, QLineSeries*> ProcessesTableModel::getMemSeriesIterator()
{
    return QMapIterator<DWORD, QLineSeries*>(memSeries);
}

// Получить серии данных о потреблении памяти заданным процессом:
QLineSeries* ProcessesTableModel::getMemSeriesForPid(DWORD pid)
{
    QMap<DWORD, QLineSeries*>::const_iterator valueIt = memSeries.find(pid);
...
QLineSeries* ProcessesTableModel::getThreadCountSeriesForPid(DWORD pid)
{
    QMap<DWORD, QLineSeries*>::const_iterator valueIt = threadCountSeries.find(pid);
...
{
    QLineSeries *series = getMemSeriesForPid(pid);
...
{
    QLineSeries *series = getThreadCountSeriesForPid(pid);
